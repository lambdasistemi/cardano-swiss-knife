...
const normalizeResolvedLabelRows = (result, matchesResult) => {
  ...
  return bindings.map((binding) => {
    ...
    return {
      ...
      typeIri: bindingValue(binding.type),
      entity,
      matched:
...
export const queryReviewResolvedLabelsImpl = (left) => (right) => (txGraphTtl) => (bookTurtles) => () => {
  try {
    return right(resolveReviewLabels(globalThis.rdfShapes, txGraphTtl, bookTurtles));
  } catch (err) {
    return left(errText(err));
  }
};
...
// Host-neutral facade over the pinned engine; hosts provide transport only.
export const resolveLabels = (rdfShapes, graphTtl) => {
  const result = rdfShapes.query(graphTtl, resolvedLabelsQuery);
  const matches = rdfShapes.query(graphTtl, resolvedLabelMatchesQuery);
  return normalizeResolvedLabelRows(result, matches);
};

const reviewResolvedLabelsQuery = `
PREFIX cardano: <https://lambdasistemi.github.io/cardano-ledger-rdf/vocab/cardano#>
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
PREFIX overlay: <https://lambdasistemi.github.io/cardano-ledger-inspector/overlay/amaru-treasury#>
SELECT DISTINCT ?label ?entity ?type ?scriptRole ?txOutRef ?fromTxOutRef ?bech32 ?slug
WHERE {
  ?entity rdfs:label ?label .
  FILTER(
    STRSTARTS(STR(?entity), "https://lambdasistemi.github.io/cardano-ledger-inspector/overlay/amaru-treasury#")
...
const normalizeReviewLabelRows = (result) => {
  if (!result || result.kind !== "solutions") {
    throw new Error("query did not return solution rows");
  }
...
export const resolveReviewLabels = (rdfShapes, txGraphTtl, bookTurtles) => {
  const rows = [];
  for (const bookTtl of bookTurtles) {
    const result = rdfShapes.query(`${txGraphTtl}\n${bookTtl}`, reviewResolvedLabelsQuery);
    rows.push(...normalizeReviewLabelRows(result));
  }
  return rows;
};

const producerOutRefsQuery = `
PREFIX cardano: <https://lambdasistemi.github.io/cardano-ledger-rdf/vocab/cardano#>
SELECT ?section ?txIdHex ?index
WHERE {
  ?transaction a cardano:Transaction .
  {
    ?transaction cardano:hasInput ?entity .
  ...
  ?entity cardano:fromTxOutRef ?ref .
  ?ref cardano:hasIndex ?index .
  ?ref cardano:hasTxId ?txId .
  ?txId cardano:bytesHex ?txIdHex .
}
ORDER BY ?section ?txIdHex ?index
`;

export const transactionInputOutRefs = (rdfShapes, graphTtl) => {
  const result = rdfShapes.query(graphTtl, producerOutRefsQuery);
  if (!result || result.kind !== "solutions") {
    throw new Error("query did not return solution rows");
  }
  const bindings = result.json?.results?.bindings;
  ...
  for (const binding of bindings) {
    const txIn = { tx_id: bindingValue(binding.txIdHex), index: Number(bindingValue(binding.index)) };
    if (bindingValue(binding.section) === "Inputs") inputs.push(txIn);
    else referenceInputs.push(txIn);
  }
  return { inputs, reference_inputs: referenceInputs };
};
