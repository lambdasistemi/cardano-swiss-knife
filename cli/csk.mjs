...
// Runtime imports stay deferred so --version works in pristine tag checkouts
// without node_modules or output/; hoisting them silently breaks both publish jobs.
const addCredentialFile = async (...args) => (await import("./vault-host.mjs")).addCredentialFile(...args);
const createVaultFile = async (...args) => (await import("./vault-host.mjs")).createVaultFile(...args);
const listVaultFile = async (...args) => (await import("./vault-host.mjs")).listVaultFile(...args);
const migrateVaultFile = async (...args) => (await import("./vault-host.mjs")).migrateVaultFile(...args);
...
const parse = async (args) => {
  ...
  if (group === "vault" && command === "credential") {
    ...
    const { invalidMetadataText, providerCredentialKind } = await import("./vault-host.mjs");
...
const secret = async (o, kind) => { const forms = [o["secret-stdin"], o["secret-fd"], o.vault].filter(Boolean); if (forms.length !== 1 || (o.vault && !o["vault-entry"])) throw secretFailure(); if (o["secret-stdin"]) return resolvedSecret(await fdText(0)); if (o["secret-fd"]) { if (!/^\d+$/.test(o["secret-fd"])) throw secretFailure(); try { return resolvedSecret(await fdText(o["secret-fd"])); } catch { throw secretFailure(); } } const { decryptVault } = await import("../lib/src/Cardano/Vault.js"); try { const passphrase = o["passphrase-fd"] ? await fdText(o["passphrase-fd"]) : await tty("Vault passphrase"); const vault = await decryptVault(passphrase, new Uint8Array(await readFile(o.vault))); const entry = vault.cardanoSwissKnifeVault.entries.filter((item) => item.id === o["vault-entry"])[0]; if (!entry || !(Array.isArray(kind) ? kind.includes(entry.kind) : entry.kind === kind) || typeof entry.value !== "string") throw Error(); return resolvedSecret(entry.value); } catch { throw secretFailure(); } };
...
const offline = async (args) => { const [family, command, subcommand, ...tail] = args; const hasSubcommand = family === "key" && ["address", "restore"].includes(command); const o = options(hasSubcommand ? tail : [subcommand, ...tail].filter((x) => x !== undefined)); const json = o.output === "json"; delete o.output; if (o.mnemonic || o["signing-key"]) offlineUsage(); let result; if (family === "address" && command === "inspect") result = await (await import("../node/src/commands/address.js")).inspect({ address: o.address }); else if (family === "mnemonic" && command === "generate") result = await (await import("../node/src/commands/mnemonic.js")).generate({ wordCount: o["word-count"] }); else if (family === "mnemonic" && command === "validate") result = await (await import("../node/src/commands/mnemonic.js")).validate({ mnemonic: await secret(o, "mnemonic") }); else if (family === "key" && command === "derive") result = await (await import("../node/src/commands/key.js")).derive({ mnemonic: await secret(o, "mnemonic"), accountIndex: o["account-index"], role: o.role, addressIndex: o["address-index"] }); else if (family === "key" && command === "address" && subcommand === "shelley") result = await (await import("../node/src/commands/key.js")).shelley({ network: o.network, paymentXpub: o["payment-xpub"], stakeXpub: o["stake-xpub"] }); else if (family === "key" && command === "address" && subcommand === "icarus") result = await (await import("../node/src/commands/key.js")).icarus({ network: o.network, addressXpub: o["address-xpub"] }); else if (family === "key" && command === "address" && subcommand === "byron") result = await (await import("../node/src/commands/key.js")).byron({ network: o.network, addressXpub: o["address-xpub"], rootXpub: o["root-xpub"], derivationPath: o["derivation-path"] }); else if (family === "key" && command === "restore" && subcommand === "icarus") result = await (await import("../node/src/commands/key.js")).restoreIcarus({ mnemonic: await secret(o, "mnemonic"), network: o.network, accountIndex: o["account-index"], role: o.role, addressIndex: o["address-index"] }); else if (family === "key" && command === "restore" && subcommand === "byron") result = await (await import("../node/src/commands/key.js")).restoreByron({ mnemonic: await secret(o, "mnemonic"), network: o.network, accountIndex: o["account-index"], addressIndex: o["address-index"] }); else if (family === "script" && command === "inspect") result = await (await import("../node/src/commands/script.js")).inspect({ cborHex: o["cbor-hex"] }); else if (family === "script" && command === "author") result = await (await import("../node/src/commands/script.js")).author({ json: o.json }); else if (family === "script" && command === "template") result = await (await import("../node/src/commands/script.js")).template({ json: o.json }); else if (family === "payload" && command === "sign") result = await (await import("../node/src/commands/payload.js")).sign({ signingKey: await secret(o, "signing-key"), payloadMode: o["payload-mode"], payloadInput: o["payload-input"] }); else if (family === "payload" && command === "verify") result = await (await import("../node/src/commands/payload.js")).verify({ payloadMode: o["payload-mode"], payloadInput: o["payload-input"], verificationKey: o["verification-key"], signature: o.signature }); else offlineUsage(); process.exitCode = render(result, json); };
...
const submitTransaction = async (args) => {
  ...
  const tx = await import("../node/src/commands/tx.js");
...
const transaction = async (args) => {
  ...
  const values = { book: [], protocolBook: [], userBook: [] }; const allowed = new Set(["--cbor-hex", "--tx-file", "--tx-hash", "--provider", "--network", "--vault", "--vault-entry", "--passphrase-fd", "--book", "--protocol-book", "--user-book", "--path", "--output", "--witness-file", "--replace-existing", "--tx-out", "--witness-out"]);
  const repeatable = new Set(["book", "protocol-book", "user-book"]);
  for (let i = 0; i < rest.length; i += 1) { const flag = rest[i]; const key = flag.slice(2); if (!allowed.has(flag) || (!repeatable.has(key) && values[key] !== undefined)) offlineUsage(); if (flag === "--replace-existing") { if (!attach) offlineUsage(); values[key] = true; continue; } if (!rest[i + 1] || rest[i + 1].startsWith("--")) offlineUsage(); const value = rest[++i]; if (key === "protocol-book") values.protocolBook.push(value); else if (key === "user-book") values.userBook.push(value); else if (key === "book") { values.book.push(value); values.userBook.push(value); } else values[key] = value; }
  if (command !== "review" && (rest.includes("--protocol-book") || rest.includes("--user-book"))) offlineUsage();
  ...
  let books; if (command !== "review") try { books = await Promise.all(values.book.map((path) => readFile(path, "utf8"))); } catch { throw txFailure("BOOK_IMPORT", "Transaction book input is invalid."); }
