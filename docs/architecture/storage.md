# Storage and Secrets

## Static assets

The browser bundle and its three hashed WASM families—address operations, ledger inspection, and RDF-shapes validation—are served as static files from the unified Nix artifact.

## In-memory state

The application keeps page state, decoded transaction results, and temporary secret inputs in memory while the session is active.

## Encrypted vault

The encrypted vault is a portable binary age v1 (`.age`) file whose decrypted
in-memory payload is the shared `cardanoSwissKnifeVault` v1 contract. Current
vault entry kinds include:

- mnemonic phrases
- signing keys
- Shelley root, account, address, and stake private keys
- Blockfrost project IDs
- Koios bearer tokens

The vault is explicit by design:

- create, unlock, persist, or download it as a `.age` file with a passphrase
- use the separate **Migrate legacy vault** action for the former JSON envelope;
  ordinary open accepts canonical age files only
- save compatible entries from feature pages
- peek or pop entries into another page when needed

## What is not persisted

Provider secrets, passphrases, and decrypted vault data are not meant to live in
browser-local storage. If a credential should survive a page reload, it should
be saved into the encrypted vault instead.

## Operational consequence

Because the product is browser-first and local-first, losing the vault passphrase or the exported vault file means losing access to the encrypted secret inventory. Backups are the user's responsibility.
