...
test("bundle JSON is rejected without mutating the local book store", async ({
  page,
}) => {
  await page.goto("/library");
  const library = page.locator(".library-page");
  ...
  await expect(
    library.getByText(
      "Book import failed: unsupported JSON kind: amaru.book.bundle.v1.",
      { exact: true },
    ),
  ).toBeVisible();
  expect(
