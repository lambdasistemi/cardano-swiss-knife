...
async function decryptVaultDocument(documentText, secret = passphrase) {
  ...
  const salt = Buffer.from(document.kdf.saltBase64, "base64");
  const iv = Buffer.from(document.cipher.ivBase64, "base64");
  expect(salt).toHaveLength(16);
  expect(iv).toHaveLength(12);
  const key = await deriveVaultKey(secret, salt, ["decrypt"]);
  const plaintext = await webcrypto.subtle.decrypt(
    { name: "AES-GCM", iv },
    key,
    Buffer.from(document.cipher.ciphertextBase64, "base64"),
  );
...
test("vault direct entry creates, opens, exports, locks, and reimports the legacy v1 format", async ({
  page,
}, testInfo) => {
  ...
  const createdPayload = await decryptVaultDocument(await readFile(createdPath, "utf8"));
  expect(createdPayload).toEqual({ version: 1, entries: [] });
  ...
  const exportedPayload = await decryptVaultDocument(exportedText);
  expect(exportedPayload).toEqual({ version: 1, entries: vaultEntries });

  const stableExport = testInfo.outputPath("round-trip-vault.json");
  await writeFile(stableExport, exportedText);
  await page.getByRole("button", { name: "Lock vault" }).click();
  await page.getByLabel("Vault passphrase").fill(passphrase);
  const chooserPromise = page.waitForEvent("filechooser");
  await page.getByRole("button", { name: "Open vault" }).click();
...
test("vault shelves filter compatible kinds and support peek and pop without clipboard transfer", async ({
  ...
  await page.goto("/vault");
  await openVaultWithChooser(page, await legacyVaultDocument(vaultEntries));
...
test("vault refuses locked saves and reports wrong passphrase, invalid document, and write failures without leaks", async ({
  ...
  const encrypted = await legacyVaultDocument(vaultEntries);
  await openVaultWithChooser(page, encrypted, "wrong passphrase");
  await expect(page.getByRole("alert")).toContainText("Vault unlock failed. Check the passphrase.");
  ...
  await expect(page.getByRole("alert")).toContainText("Vault file format is not supported.");
  ...
  await page.evaluate((documentText) => {
    window.showOpenFilePicker = async () => [
      {
        name: "write-failure.vault.json",
        getFile: async () => new File([documentText], "write-failure.vault.json", { type: "application/json" }),
  ...
  await expect(page.getByRole("alert")).toContainText("Vault save failed: simulated disk full");
