...
pkgs.runCommand "cardano-swiss-knife-node-api-check" {
  ...
  cd work
  CSK_PACKAGE_TARBALL="$(echo ${nodePackage}/*.tgz)" node --test node/test/api.test.mjs node/test/transaction-api.test.mjs node/test/transaction-provider.test.mjs node/test/transaction-books.test.mjs
  mkdir -p $out
''
