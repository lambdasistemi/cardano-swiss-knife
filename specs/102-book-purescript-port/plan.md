...
- `docs/inspector/spago.lock` (only the matching local-package closure required by the inspector workspace)
...
- No new npm dependency is permitted; only registry-pinned PureScript packages may be declared. The parent authorized the minimal root and inspector `spago.lock` closure updates required by `argonaut-core` and `foreign-object`; unrelated lock churn is rejected.
