...
- [X] T004 [US1] Add RED contract coverage for Blockfrost and Koios successful
  CBOR and validation-context loads in `test/src/Test/Provider.purs`.
- [X] T005 [US1] Add RED contract coverage for endpoint/auth/network mapping and
...
- [X] T006 [US1] Implement shared provider/network types, typed failures,
  request selection, Fetch IO, decoding, and context mapping in
  `lib/src/Cardano/Provider.purs` and `lib/src/Cardano/Provider.js`.
- [X] T007 [US1] Preserve selected-provider-only producer/context resolution and
  explicit failure behavior in the shared core.
- [X] T008 [US2] Convert WebUI `Provider`/`FFI.Blockfrost` modules to thin shared
  adapters and delete `Provider.js` plus Koios/Blockfrost request FFI files.
- [X] T009 [US2] Prove `nix run .#ci-test`, inspector build, and existing provider
  Playwright journeys are GREEN with no visible behavior change.
- [X] T010 [US1] Obtain navigator RED/GREEN approval, run `./gate.sh`, and commit
...
- [X] T011 [US3] Add a RED architecture-boundary check in
  `scripts/check-architecture-boundary.sh` and wire it into `gate.sh`.
- [X] T012 [US3] Make the check enforce the single shared endpoint/auth location,
  no HTTP in compatibility modules, and no host semantic/fallback dependencies.
- [X] T013 [US3] Add negative self-tests proving representative duplicate
  provider and forbidden dependency inputs are rejected.
- [X] T014 [US3] Expand `docs/architecture/system.md` with the host/engine
...
- [X] T015 [US3] Obtain navigator RED/GREEN approval and prove the focused check,
  strict MkDocs build, `nix develop --quiet -c just ci`, and `./gate.sh` are
  GREEN.
- [X] T016 Finalize task accounting, commit audit, PR body, push, and ready state
