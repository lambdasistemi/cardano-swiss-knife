...
- [X] T003 Add the RED Playwright proof for the exact 12-present/9-absent
  direct body-field partition on the existing treasury fixture.
- [X] T004 Assert the engine-derived ttl, withdrawal, and signer values plus
  `network_compliance scope owner` on the B-Labeled required-signer row.
- [X] T005 Replace the three unconditional null builders with minimal
  RDF-backed rows that remain null only when their predicates are absent.
- [X] T006 Obtain navigator RED and GREEN approvals, run
