...
- [ ] T005-S1 Wire the offline property file into `ci-node-api` while retaining every example test.
...
- [ ] T008-S2 Add commented RED→GREEN properties for `inspectTransaction`, `browseTransaction`, `identifyTransaction`, and `transactionIntent`.
...
- [ ] T012-S2 Wire the transaction property file into `ci-node-api` while retaining every prior test.
...
- [ ] T015-S3 Add commented RED→GREEN properties for all six witness-planning, preparation, normalization, attachment, validation, and evaluation exports.
...
- [ ] T019-S3 Wire the ledger property file into `ci-node-api` and add the README pointer to the executable contracts, including the #77 follow-up gap.
